...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_new_password)) {
        ...
        $query = "SELECT password FROM Patient WHERE num_carte = :num_carte";
        $stmt = $db->prepare($query);
        $stmt->execute(['num_carte' => $patient['num_carte']]);
        $stored_password = $stmt->fetchColumn();

        if (password_verify($current_password, $stored_password)) {
            $new_password_hash = password_hash($new_password, PASSWORD_DEFAULT);
            $query = "UPDATE Patient SET password = :password WHERE num_carte = :num_carte";
            $stmt = $db->prepare($query);
            $stmt->execute(['password' => $new_password_hash, 'num_carte' => $patient['num_carte']]);
...
<style>
    ...
    <form method="post">
        ...
        <br>
        <input type="submit" value="Modifier" class="btn btn-primary py-2 px-4 ms-3">
    </form>

    <br><br><br>

    <p><a href="dashboard_patient.php" class="btn btn-primary py-2 px-4 ms-3">Retour au tableau de bord</a></p>
